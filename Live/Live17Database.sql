...
select marital,sum(balance),avg(balance) from bank_details group by marital
select marital, sum(balance),avg(balance) from bank_details group by marital having sum(balance)>300
set sql_safe_updates=0
update bank_details set balance =0 where job='unknown'
update bank_details set contact='known' , y='yes' where month='may'
update bank_details set `default`=NULL where `default`=''
...
DELIMITER &&
create procedure select_pre_filter1(IN var int,IN var1 varchar(30))
BEGIN
	select * from bank_details where job=var1 and balance>var;
END &&
call select_pre_filter1(100,'services')
